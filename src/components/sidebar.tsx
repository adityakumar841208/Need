...
import { BiHomeAlt2, BiChat, BiUser, BiCog, BiLogOut, BiMenu } from 'react-icons/bi'
...
const NavItem: React.FC<NavItemProps> = ({ icon, text, isOpen, path }) => {
    ...
    const isActive = '/home' + path === pathname || path === '/' && pathname === '/home' || path === '/chat' && pathname.includes('/chat')
            ...
            <button
                ...
                className={`w-full flex items-center p-3 rounded-lg transition-all duration-300
                    ${isActive
                        ...
                        : 'hover:bg-slate-100 text-slate-600 dark:text-white dark:hover:text-slate-900 hover:translate-x-1'}`}>
                <span>{icon}</span>
